...
## 概要
モックログイン認証サービスの実装。SaaSアプリケーション向けに、ログイン画面を提供し、ハードコードされたJWTトークンを返却する。フロントエンド中心の実装で、サーバーレス・低コストを優先する。

## 技術コンテキスト
**言語/バージョン**: HTML5/CSS3/JavaScript (ES6+)、静的サイトとして実装
**主要依存関係**: なし（Pure JavaScript、外部ライブラリ不要）またはjsonwebtoken（JWT生成用、CDN経由）
**ストレージ**: N/A（モックサービスのためデータ永続化なし）
**テスト**: ブラウザ手動テスト（モックサービスのため自動テストは不要）
**ターゲットプラットフォーム**: モダンブラウザ（Chrome, Firefox, Safari, Edge最新版）
**プロジェクトタイプ**: Web（フロントエンドのみ）- 静的HTMLページ
**パフォーマンス目標**: <1秒でJWT生成とリダイレクト
**制約**: サーバーレス、低コスト、フロントエンドのみで完結
**スケール/スコープ**: 1ページ（ログイン画面）、モック用途のみ
...
```
auth-app/
├── index.html           # ログイン画面のHTMLファイル
├── styles.css           # スタイルシート（UI/UXデザイン）
├── auth.js              # JWT生成とリダイレクト処理
└── README.md            # アプリケーションの使用方法
```

**構造決定**: フロントエンドのみのシンプルな構成を採用。静的HTMLファイルとして配信可能で、GitHub Pages、Netlify、Vercelなどの無料ホスティングサービスで動作する。
